...
#This is a proposed solution#######
    def _get_opt(self, optimizer: str = None, lr: float = 0.001):
        """Get the optimizer according to configuration. Usually we will use Adam.
        
        Args:
            optimizer (str): Name of the optimizer. Defaults to "adam".
            lr (float): Learning rate for the optimizer.
            
        Returns:
            Optimizer object.
        """
        # Dictionary for extensibility
        optimizer_dict = {
        ...
        # Automatically handle compatibility for Adam optimizer on M1/M2 Macs
        if optimizer == "adam":
            try:
                # Attempt to use the legacy optimizer for compatibility
                return tf.keras.optimizers.legacy.Adam(learning_rate=lr)
            except AttributeError:
                # Fallback to regular Adam if legacy is not available
                return tf.keras.optimizers.Adam(learning_rate=lr)
        
        # Extendable logic for other optimizers
        if optimizer in optimizer_dict:
            return optimizer_dict[optimizer](learning_rate=lr)
        else:
